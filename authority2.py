...
def mine_block():
    ...
    response = {'message': 'Congratulations,block mined! by (Authority2)',
                'index': block['index'],
...
def is_valid():
    ...
    if is_valid:
        response = {'message': ' (Authority 2)- All good. The Blockchain is valid.'}
    else:
        response = {
            'message': '(Authority 2)-we have a problem. The Blockchain is not valid.'}
    return jsonify(response), 200
...
def add_transaction():
    ...
    if not all(key in json for key in transaction_keys):
        return 'Some elements of the transaction are missing in (Authority 2)', 400
    index = blockchain.add_transaction(
        json['uid'], json['name'], json['pid'], json['court_id'], json['district'], json['section'], json['state'])
    response = {
        'message': f'This transaction will be added to Block {index} by (Authority 2)'}
...
def connect_node():
    ...
    if nodes is None:
        return "Alert ! No node found. failed to connect (Authority 2)", 400
    for node in nodes:
        blockchain.add_node(node)
    response = {'message': 'Node Connection established! (Authority 2)',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'Chain updated in (Authority2) !!',
                    'new_chain': blockchain.chain}
    else:
        response = {'message': 'The chain is the largest one (Authority2).',
